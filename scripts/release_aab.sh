...
# release_aab.sh
# Automates the process of building a release App Bundle (.aab) for Play Store
...
echo "🚀 Starting Play Store Release Build (AAB)..."

# 1. Check for Keytool
if ! command -v keytool &> /dev/null; then
    echo "⚠️ Warning: 'keytool' not found."
else
    echo "✅ Keytool found."
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "👉 Ushbu faylni Google Play Console ga yuklaysiz."
else
    echo "❌ Build Failed."
    exit 1
fi
