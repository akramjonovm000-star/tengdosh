...
async def verify():
    print('--- Testing Student Search by ID Param ---')
    ...
    # 2. Test searching by params={'id': target_id}
    print(f'\n2. Test "id" filter: "{target_id}"')
    r2 = await HemisService.fetch_with_retry(client, 'GET', url, 
        params={'limit': 5, 'id': target_id}, 
        headers={'Authorization': f'Bearer {token}'})
    ...
    # 3. Test searching by params={'student_id': target_id}
    print(f'\n3. Test "student_id" filter: "{target_id}"')
    r3 = await HemisService.fetch_with_retry(client, 'GET', url, 
        params={'limit': 5, 'student_id': target_id}, 
        headers={'Authorization': f'Bearer {token}'})
