...
class HemisService:
    ...
    async def get_curriculum_topics(token: str, subject_id: str = None, semester_code: str = None, training_type_code: str = None, student_id: int = None):
        """
        Fetch master list of topics for a curriculum subject.
        Endpoint: /v1/data/curriculum-subject-topic-list
        """
        ...
        if student_id:
            try:
                async with AsyncSessionLocal() as session:
                    ...
                    if cache:
                        age = (datetime.utcnow() - cache.updated_at).total_seconds()
                        if age < 24 * 3600: # 24h TTL for curriculum topics (they don't change often)
                            return cache.data
                        stale_data = cache.data
            except: pass
        ...
        async with httpx.AsyncClient(verify=False) as client:
            try:
                ...
                response = await client.get(url, headers=HemisService.get_headers(token), params=params, timeout=12)
                ...
                logger.error(f"Curriculum Topics Error: {e}")
                return stale_data or []
