...
async def cb_ai_ask(call: CallbackQuery, state: FSMContext):
    logger.info(f"AI ASK Triggered: {call.data}")
    await call.answer()
    
    # Set state to isolate user from global login handler
    await state.set_state(AIStates.viewing_credit_system)
...
# ISOLATED HANDLER FOR CREDIT SYSTEM STATE
@router.message(AIStates.viewing_credit_system)
async def handle_credit_system_text(message: Message, state: FSMContext):
    # Whatever they type, we just show the Back button.
    # We do NOT treat it as login or password.
